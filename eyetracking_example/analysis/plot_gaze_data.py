...
fp = os.path.join('../results/', observer, 'EDF', '%s00.asc' % observer)

# read gaze data
edfdata = read_edf(fp, EDFSTART, stop=EDFSTOP, missing=INVALCODE, debug=True)

# read responses
data = pd.read_csv('../results/demo/block_0.csv')
...
for trialnr in range(len(data)):
    ...
    imagefile = '../stimuli/demo/block_0_%d.png' % trialnr
    ...
    # paths
    rawplotfile = os.path.join(pplotdir, "raw_data_%s_%d" % (observer, trialnr))
    scatterfile = os.path.join(pplotdir, "fixations_%s_%d" % (observer, trialnr))
    scanpathfile =  os.path.join(pplotdir, "scanpath_%s_%d" % (observer, trialnr))
    heatmapfile = os.path.join(pplotdir, "heatmap_%s_%d" % (observer, trialnr))
