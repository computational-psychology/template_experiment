...
# -*- coding: utf-8 -*-
"""
MLDS experiment  for perceived transmittance with varying physical color of the
transparency, at differente physical transmittance
...
if __name__ == '__main__':
    ...
    # reads block order
    blockorder = read_design('design/%s/mlds/%s_experiment_order.txt' %(vp_id, vp_id))
    
    # reads the blocks already done
    try:
        blocksdone = read_design('results/%s/mlds/%s_blocks_done.txt' %(vp_id, vp_id))
    ...
    for i in range(len(blockstorun['number'])):
        ...
        # log file name and location
        design = read_design_csv('design/%s/mlds/block_%d.csv' %(vp_id, sess))
        rfl    = open('results/%s/mlds/%s_block_%d.txt' %(vp_id, vp_id, sess), 'a')
        ...
        if start_trl == 0:
            result_headers = ['block', 'trial', 'alpha', 'tau1', 'tau2', 'tau3', 'Response', 'resp.time', 'timestamp']
